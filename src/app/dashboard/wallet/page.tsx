...
export default async function WalletPage() {
    ...
    const [stats, transactions, pendingPayout, redemptions] = await Promise.all([
        ...
        query<any>(
            "SELECT id, amount, status FROM payout_requests WHERE architect_id = ? AND status IN ('PENDING', 'IN_PAYMENT', 'HOLD') LIMIT 1",
            [session.user.id]
        ),
        getMyRedemptions()
    ]);

    const hasPendingRequest = pendingPayout.length > 0;
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-10">
                ...
                <div className="stat-card bg-emerald-500/5 border border-emerald-500/10 flex flex-col justify-between p-10 relative group">
                    ...
                    <div className="relative z-10">
                        ...
                        {hasPendingRequest ? (
                            <div className={`flex items-center gap-3 p-4 rounded-2xl border mb-8 ${pendingPayout[0].status === 'HOLD' ? 'bg-amber-500/5 border-amber-500/20' : 'bg-blue-500/5 border-blue-500/20'
                                }`}>
                                <AlertCircle size={16} className={pendingPayout[0].status === 'HOLD' ? 'text-amber-500' : 'text-blue-600'} />
                                <p className={`text-[10px] font-bold uppercase tracking-widest leading-relaxed ${pendingPayout[0].status === 'HOLD' ? 'text-amber-500' : 'text-blue-600'
                                    }`}>
                                    {pendingPayout[0].status === 'IN_PAYMENT'
                                        ? `Wypłata ${formatPLN(pendingPayout[0].amount)} PLN jest w trakcie realizacji.`
                                        : pendingPayout[0].status === 'HOLD'
                                            ? `Wniosek o wypłatę ${formatPLN(pendingPayout[0].amount)} PLN został wstrzymany.`
                                            : `Wniosek o wypłatę ${formatPLN(pendingPayout[0].amount)} PLN oczekuje na zatwierdzenie.`
                                    }
                                </p>
                            </div>
                        ) : (
                    ...
                    <div className="stat-card bg-card p-8 border border-black/5">
                        <div className="flex items-center gap-4 p-4 rounded-2xl bg-black/5 border border-black/5 group hover:border-brand-primary/30 transition-all cursor-pointer">
                            ...
                            <div>
                                ...
                                <p className="text-[10px] text-stone-600 font-bold group-hover:text-stone-400 transition-all">finanse@antigravity.pl</p>
                            </div>
                        </div>
                    </div>
                </div>
